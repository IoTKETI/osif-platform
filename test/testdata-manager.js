...
var compositedVoManager = require("../backend/managers/composited-vo-manager");
...
var testMakerTypes = [
  ...
  'fas fa-ruler-combined darkgray',
  'fas fa-leaf grass',
  'fas fa-lemon grass',
...
function __addControlNodes(compositedVO) {
  ...
  compositedVO.nodeList.splice(0, 0, node);
  ...
  for(var i=0; i < randomNumber; i++) {
    ...
    var index = _randomNumber(compositedVO.nodeList.length -1) + 1;

    compositedVO.nodeList.splice(index, 0, node);
  }
  ...
  compositedVO.nodeList.push(node);
...
function __testLinkSequantial(compositedVO) {
  ...
  compositedVO.nodeList[0].nodePosition = {x: x, y: y};

  for(var i=0; i < compositedVO.nodeList.length-1; i++) {
    ...
    compositedVO.nodeList[i+1].nodePosition = {x: x, y: y};

    var source = compositedVO.nodeList[i];
    var target = compositedVO.nodeList[i+1];
  ...
  try {
      ...
      for( var i=1; i <= test_NUMBER_OF_TEST_AE; i++ ) {
        ...
        for(var j=0; j < cntCount; j++) {
          ...
          var container = {
            ...
            "virtualObject": {
              "voId": "cnt-" + __cntIndex,
              "voName": cntName + "-" + __cntIndex,
              "resourceUrl": "/" + ('ae-' + i) + "/" + (cntName + "-" + __cntIndex),
              "icon": testVOIconTypes[__cntIndex % testVOIconTypes.length],
              "iconColor": testIconColor[__cntIndex % testIconColor.length],
              "voLatestValue": lastValue,
            }
          };

          testVOList.push(container.virtualObject);
    ...
    if(testCVOList == null ) {
      ...
      for (var id = 1; id <= test_NUMBER_OF_TEST_CVO; id++) {
        ...
        var compositedVO = {
          "cvoId": 'cvo-' + id,
          "cvoName": cvoNames[_randomNumber(cvoNames.length)] + ' - ' +id,
          "icon": testVOIconTypes[id % testVOIconTypes.length],
          "iconColor": testIconColor[id % testIconColor.length],
          "cvoStatus": 'RUNNING',
          "nodeList": []
        };
        ...
        for (var i = 0; i <= voCount; i++) {
          ...
          var nodeIndex = compositedVO.nodeList.indexOf(function(item){
            if(item.nodeData == vo)
              return true;
            else
              return false;
    ...
    if(testVirtualSpaceList == null) {
      ...
      for(var id=1; id <= 10; id++) {
        ...
        for (var j = 1; j <= voCount; j++) {
          ...
          var voNodeList = cvo.nodeList.filter(function(item){
            if(item.nodeType == 'vo')
              return true;
            else
              return false;
